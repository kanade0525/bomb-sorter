...
歩き回る「ボムすけ」を色で仕分ける、スマホ向けのタッチアクションゲームです。
**縦持ちでも横持ちでも遊べます。**
...
**縦持ちでも横持ちでも遊べます。** 向きに応じて箱の位置が変わります。

| 向き   | 箱の位置                | 向いている遊び方                     |
| ------ | ----------------------- | ------------------------------------ |
| 縦持ち | 画面の**下**に 2 つ並ぶ | 片手で、親指の可動域だけで振り分ける |
| 横持ち | 画面の**左右**の端      | 両手で持って、左右へ大きく振り分ける |

どちらでも短い方の辺を 360 に固定しているので、ボムすけの大きさも当たり判定も難易度も同じです。
記録は向きによらず公平に比べられます。
...
- 数体から始まって、**短い間隔でどんどん湧いてきます**
- 指でドラッグして、**同じ色の箱**へ運びます（左が赤、右が黒。向きが変わっても並びは同じ）
- 違う色の箱に入れると爆発してゲームオーバー
- 導火線が尽きても爆発。頭の上のドットが残り時間です
- 続けて成功すると連鎖して倍率が上がります。最後の成功から 3 秒以内に次を決めると継続
- 時間が経つほど難しくなります。効くのは 4 つで、出現の間隔・同時に存在できる数・
  導火線の長さ・歩く速さです

| 経過   | 出現間隔 | 同時上限 | 導火線  | 歩く速さ |
| ------ | -------- | -------- | ------- | -------- |
| 0 秒   | 0.70 秒  | 6 体     | 11.0 秒 | 1.00 倍  |
| 30 秒  | 0.51 秒  | 9 体     | 10.2 秒 | 1.38 倍  |
| 60 秒  | 0.41 秒  | 12 体    | 9.4 秒  | 1.75 倍  |
| 120 秒 | 0.33 秒  | 18 体    | 7.8 秒  | 2.50 倍  |
| 240 秒 | 0.30 秒  | 18 体    | 4.6 秒  | 2.80 倍  |

どのくらい生き残れるかは、自動で捌く相手を走らせて実測しました。
「1 回の仕分けにかかる時間」を変えて、腕前の違いを模したものです。

| 1 回の仕分け                        | 生存   | 得点      | 最大同時      |
| ----------------------------------- | ------ | --------- | ------------- |
| 0.90 秒（初見）                     | 56 秒  | 約 3 万   | 12 体         |
| 0.65 秒（普通）                     | 82 秒  | 約 6.5 万 | 15 体         |
| 0.45 秒（慣れ）                     | 112 秒 | 約 13 万  | 18 体（上限） |
| 0.30 秒（上級）                     | 196 秒 | 約 33 万  | 18 体         |
| 0.20 秒（人が続けるのは無理な速さ） | 259 秒 | 約 60 万  | 18 体         |

**難易度の天井を決めているのは同時上限ではなく出現間隔です。** 上限に達するのは
「捌くのが湧きに追いつかなくなったあと」なので、湧きより速く捌ける相手は上限に触れないまま
永久に生き残ってしまいます（実際に 900 秒生き延びられました）。得点を競う遊びなので、
終盤の出現間隔を詰めて、いつかは必ず終わるようにしてあります。

数値はすべて [`src/core/constants.ts`](src/core/constants.ts) の 1 箇所にあります。
...
## YouTube ゲームルーム（Playables）向け

応募・審査に使う資料は [`docs/submission/youtube-playables.md`](docs/submission/youtube-playables.md)
にまとめてあります（英語）。紹介文、実測値つきの技術仕様、公開されている認定要件との
突き合わせ、スクリーンショット一式。

ゲームルーム向けのビルドは公開中のサイトとは別で、`npm run build:playables` で作ります。
違いは次の 3 点です。

- 絶対パスが禁じられているので `base` を相対にする
- SDK をゲームコードより前に読み込み、`firstFrameReady()` → `gameReady()` を通知する
- Service Worker とアイコン、リンクのプレビュー画像を含めない（配信も操作も向こうが持つ）

音量・一時停止・保存・記録の送り先はプラットフォームに従い、ゲーム内のミュート・
一時停止・全画面のボタンは出しません（プラットフォームの操作ボタンに似たものを置くことは
認定要件で禁じられています）。場の違いは
[`src/platform/host.ts`](src/platform/host.ts) の一枚に閉じてあり、SDK が無いときは
